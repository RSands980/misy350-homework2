        ...
        if place_btn:
            with st.spinner("Placing order..."):
                ...
                if customer_name == "":
                    ...
                    selected_item["stock"] = selected_item["stock"] - quantity
            ...
            with st.spinner("Updating stock..."):
                ...
                for item in inventory:
                    if item["name"] == restock_item:
                        item["stock"] = item["stock"] + restock_amount
    ...
    with st.container(border=True):
        ...
        if len(active_orders) == 0:
            ...
            selected_order = st.selectbox(
                ...
                active_orders,
                format_func=lambda x: f"{x['order_id']} - {x['customer']} - {x['item']} (Qty: {x['quantity']})",
                key="selected_order"
            )
                ...
                with st.spinner("Cancelling order..."):
                    ...
                    for item in inventory:
                        if item["name"] == selected_order["item"]:
                            item["stock"] = item["stock"] + selected_order["quantity"]
